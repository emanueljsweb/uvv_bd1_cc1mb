...
COMMENT ON TABLE  lojas.produtos                            IS 'Essa tabela é responsável por armazenar os dados dos produtos das Lojas UVV.';

-- Comenta as colunas da tabela produtos.
COMMENT ON COLUMN lojas.produtos.produto_id                 IS 'ID do Pedido. Essa coluna é a chave primária que identifica o pedido.';
COMMENT ON COLUMN lojas.produtos.preco_unitario             IS 'Preço unitário do produto. Essa coluna não pode ter valor menor que zero.';
...
COMMENT ON COLUMN lojas.produtos.imagem_mime_type           IS 'Tipo de dado armazenado na coluna imagem.';
COMMENT ON COLUMN lojas.produtos.imagem_arquivo             IS 'Nome do arquivo da imagem do produto. Essa coluna não pode ser nula caso a coluna imagem esteja preenchida.';
COMMENT ON COLUMN lojas.produtos.imagem_charset             IS 'Codificação de caracteres utilizada no arquivo da imagem do produto. Essa coluna não pode ser nula caso a coluna imagem esteja preenchida.';
COMMENT ON COLUMN lojas.produtos.imagem_ultima_atualizacao  IS 'Data da última atualização da imagem do produto. Essa coluna não pode ser nula caso a coluna imagem esteja preenchida.';
...
COMMENT ON TABLE  lojas.lojas                               IS 'Essa tabela é responsável por armazenar os dados das lojas da rede de Lojas UVV.';

-- Comenta as colunas da tabela lojas.
COMMENT ON COLUMN lojas.lojas.loja_id                       IS 'ID da Loja. Essa coluna é a chave primária que identifica a loja.';
COMMENT ON COLUMN lojas.lojas.nome                          IS 'Nome da Loja. Essa coluna não pode ser nula.';
COMMENT ON COLUMN lojas.lojas.endereco_web                  IS 'URL do Site. Essa coluna não pode ser nula se o endereço físico não estiver preenchido.';
COMMENT ON COLUMN lojas.lojas.endereco_fisico               IS 'Endereço físico da loja. Essa coluna não pode ser nula se o endereço web não estiver preenchido.';
COMMENT ON COLUMN lojas.lojas.latitude                      IS 'Latitude da localidade.';
COMMENT ON COLUMN lojas.lojas.longitude                     IS 'Longitude da localidade.';
COMMENT ON COLUMN lojas.lojas.logo                          IS 'Logotipo da loja.';
COMMENT ON COLUMN lojas.lojas.logo_mime_type                IS 'Tipo de dado armazenado na coluna logo. Essa coluna não pode ser nula caso a coluna logo esteja preenchida.';
COMMENT ON COLUMN lojas.lojas.logo_arquivo                  IS 'Nome do arquivo da logotipo da loja. Essa coluna não pode ser nula caso a coluna logo esteja preenchida.';
COMMENT ON COLUMN lojas.lojas.logo_charset                  IS 'Codificação de caracteres utilizada no arquivo da logotipo da loja. Essa coluna não pode ser nula caso a coluna logo esteja preenchida.';
COMMENT ON COLUMN lojas.lojas.logo_ultima_atualizacao       IS 'Data da última atualização de logotipo. Essa coluna não pode ser nula caso a coluna logo esteja preenchida.';
...
COMMENT ON TABLE  lojas.estoques                            IS 'Essa tabela é responsável por armazenar os dados dos estoque das Lojas UVV.';

-- Comenta as colunas da tabela estoques.
COMMENT ON COLUMN lojas.estoques.estoque_id                 IS 'ID do Estoque. Essa coluna é a chave primária que identifica o estoque.';
COMMENT ON COLUMN lojas.estoques.loja_id                    IS 'Loja responsável pelo estoque. Essa coluna é uma chave externa da tabela lojas e não pode ser nula.';
COMMENT ON COLUMN lojas.estoques.produto_id                 IS 'ID do Produto. Essa coluna é uma chave externa da tabela produtos e não pode ser nula.';
COMMENT ON COLUMN lojas.estoques.quantidade                 IS 'Quantidade em estoque. Essa coluna não pode ser nula e seu valor precisa ser maior ou igual a zero.';
...
COMMENT ON TABLE  lojas.clientes                            IS 'Essa tabela é responsável por armazenar os dados dos clientes das Lojas UVV.';

-- Comenta as colunas da tabela clientes.
COMMENT ON COLUMN lojas.clientes.cliente_id                 IS 'ID do Cliente. Essa coluna é a chave primária que identifica o cliente.';
COMMENT ON COLUMN lojas.clientes.email                      IS 'E-mail do cliente. Essa coluna não pode ser nula e precisa atender ao formato de um e-mail.';
COMMENT ON COLUMN lojas.clientes.nome                       IS 'Nome do cliente. Essa coluna não pode ser nula.';
...
COMMENT ON TABLE  lojas.envios                              IS 'Essa tabela é responsável por armazenar os dados dos envios realizados pelas Lojas UVV.';

-- Comenta as colunas da tabela envios.
COMMENT ON COLUMN lojas.envios.envio_id                     IS 'ID do Envio. Essa coluna é a chave primária que identifica o envio.';
COMMENT ON COLUMN lojas.envios.loja_id                      IS 'Loja responsável pelo envio. Essa coluna é uma chave externa da tabela lojas e não pode ser nula.';
COMMENT ON COLUMN lojas.envios.cliente_id                   IS 'Cliente destinatário do envio. Essa coluna é uma chave externa da tabela clientes e não pode ser nula.';
COMMENT ON COLUMN lojas.envios.endereco_entrega             IS 'Endereço de entrega do envio. Essa coluna não pode ser nula.';
COMMENT ON COLUMN lojas.envios.status                       IS 'Situação do pedido. Essa coluna não pode ser nula e deve ser um desses valores: CRIADO, ENVIADO, TRANSITO e ENTREGUE.';
...
COMMENT ON TABLE  lojas.pedidos                             IS 'Essa tabela é responsável por armazenar os pedidos das Lojas UVV.';

-- Comenta as colunas da tabela pedidos.
COMMENT ON COLUMN lojas.pedidos.pedido_id                   IS 'ID do Pedido. Essa coluna é a chave primária que identifica o pedido.';
COMMENT ON COLUMN lojas.pedidos.data_hora                   IS 'Data e hora de inclusão do pedido. Essa coluna não pode ser nula.';
COMMENT ON COLUMN lojas.pedidos.cliente_id                  IS 'Cliente responsável pelo pedido. Essa coluna é uma chave externa da tabela clientes e não pode ser nula.';
COMMENT ON COLUMN lojas.pedidos.status                      IS 'Situação do pedido. Essa coluna não pode ser nula e deve ser um desses valores: CANCELADO, COMPLETO, ABERTO, PAGO, REEMBOLSADO E ENVIADO.';
COMMENT ON COLUMN lojas.pedidos.loja_id                     IS 'Loja responsável pelo pedido. Essa coluna é uma chave externa da tabela lojas e não pode ser nula.';
...
COMMENT ON TABLE  lojas.pedidos_itens                       IS 'Essa tabela é responsável por armazenar os itens dos pedidos das Lojas UVV.';

-- Comenta as colunas da tabela pedidos_itens.
COMMENT ON COLUMN lojas.pedidos_itens.pedido_id             IS 'ID do Pedido. Essa coluna faz parte da chave primária que identifica os itens do pedido.';
COMMENT ON COLUMN lojas.pedidos_itens.produto_id            IS 'ID do Produto. Essa coluna faz parte da chave primária que identifica os itens do pedido.';
COMMENT ON COLUMN lojas.pedidos_itens.numero_da_linha       IS 'Número da linha do item. Essa coluna não pode ser nula.';
COMMENT ON COLUMN lojas.pedidos_itens.preco_unitario        IS 'Preço unitário do item. Essa coluna não pode ser nula e seu valor não pode ser menor que zero.';
COMMENT ON COLUMN lojas.pedidos_itens.quantidade            IS 'Quantidade do item. Essa coluna não pode ser nula e seu valor precisa ser maior que zero.';
COMMENT ON COLUMN lojas.pedidos_itens.envio_id              IS 'ID do Envio. Essa coluna é uma chave externa da tabela envios.';
